...
<body>
	...
	?>
		<form name="estudantedis" id="estudantedis" method="post">
		 	<table border="0" cellspacing="0" cellpadding="0" class="rt">
		 		<tr>
					<td>Pesquisar disciplina：</td>
					<td>
		 				<input type="text" name="sc" id="sc" size="20" required="required">
		 				<span style="color: red;font-size: 18px;">*</span>
		 			</td>
		 			<td><input type="submit" name="search" value="Pesquisar" class="sb"></td>
		 	</table>
		 </form>
		 <?php 
			...
			//obter numero disciplina
			if(isset($_POST['search']))
			{
				$ndis=$_POST['sc'];
				$result1=mysqli_query($link,"select disciplina.ndis, disciplina.dnome, professor.nome from disciplina inner join professor on disciplina.ndis=professor.ndis where disciplina.ndis='$ndis'");
				$result3=mysqli_query($link,"select * from disciplina where disciplina.ndis='$ndis'");
			...
			{
				echo "<script>alert('Nao se encontra disciplina')</script>";
			}
		}
		mysqli_close($link);
	?>			
